...
for (const [key, value] of Object.entries(dataOrg)){
    ...
    li.innerHTML = '<div class="wrapper"><span class="text">' + key + '</span>' + '<span class="arrow">></span></div>'
    list.appendChild(li);
}
...
for (let i = 0; i < obj.length ; i++) {
    obj[i].onclick = function() {
        ...
        foodMenu.forEach((element) => {
            grid.innerHTML += '<div class="one">' + '<div class="food-name">' + element["name"] +'</div>' + '<div class="food-price">' + 'Rs. '+ element["price"] + '</div>' + '<div class="btndiv"><button class="btn-black">ADD TO CART</button></div>' + '</div>'
        });
        ...
        for (let a = 0; a < btn.length ; a++) {
            btn[a].onclick = function() {
                ...
                pricing.innerHTML += '<div class="pricing-item">' + '<span class="food-cart-name">' + dataOrg[mystr][a]["name"] + '</span>' + '<div class="pricing-grid">' + '<div class="text-total">' + "Total" + '</div>' + '<div class="food-cart-price">' + 'Rs. ' + dataOrg[mystr][a]["price"] + '</div>' + '</div>' + '</div>';
                grandTotal += dataOrg[mystr][a]["price"];
                console.log(grandTotal)
                totalPrice.innerHTML = 'Total Price' + '<span class="grand-total">' + 'Rs. ' + grandTotal + '</span>'
            }
        }
    }
}
